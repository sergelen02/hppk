...
func KeyGenDS(params Params) (*Secret, *Public) {
	...
	betaMask := core.RandMod(field.P)

	rows := params.N + params.Lambda + 1
	encP := make([][]*big.Int, rows)
	encQ := make([][]*big.Int, rows)
	pPrime := make([][]*big.Int, rows)
	qPrime := make([][]*big.Int, rows)
	mu := make([][]*big.Int, rows)
	nu := make([][]*big.Int, rows)
	Rbar := core.Pow2(params.KBits)

	for i := 0; i < rows; i++ {
		encP[i] = make([]*big.Int, params.M)
		encQ[i] = make([]*big.Int, params.M)
		pPrime[i] = make([]*big.Int, params.M)
		qPrime[i] = make([]*big.Int, params.M)
		mu[i] = make([]*big.Int, params.M)
		nu[i] = make([]*big.Int, params.M)
		for j := 0; j < params.M; j++ {
			...
			pPrime[i][j] = field.Mul(betaMask, field.Normalize(encP[i][j]))
			qPrime[i][j] = field.Mul(betaMask, field.Normalize(encQ[i][j]))
			mu[i][j] = new(big.Int).Div(new(big.Int).Mul(Rbar, encP[i][j]), S1)
			nu[i][j] = new(big.Int).Div(new(big.Int).Mul(Rbar, encQ[i][j]), S2)
		}
	}

	sk := &Secret{
		F: f, H: h, R1: R1, S1: S1, R2: R2, S2: S2,
		PMatrix: encP, QMatrix: encQ, Field: field, Params: params,
	}
	pk := &Public{
		PPrime: pPrime, QPrime: qPrime, Mu: mu, Nu: nu,
		S1Bar: field.Mul(betaMask, field.Normalize(S1)),
		S2Bar: field.Mul(betaMask, field.Normalize(S2)),
